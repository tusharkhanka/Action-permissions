...
public class CustomWorkflowUtil {
	 ...
	 */
	public void sendMail(String templatePATH, List<String> recipientsList,
	...
	public List<Map<String, String>> getAttachedDocsList(NodeRef packageNodeRef) {
		...
		for (ChildAssociationRef childAssociationRef : lResources) {
			...
			if(siteInfo != null){
				...
				logger.debug("No siteInfo manjhe the doc is attached externally");
	...
	public void prepareReminderMail(String workflowDescription,
			String templatePATH, String groupName, String taskId,
			String initiator, String subject, NodeRef packageNodeRef) {
		...
		String address = null;
		if (personService.personExists(initiator) == true) {
			NodeRef person = personService.getPerson(initiator);
			address = (String) nodeService.getProperty(person,
					ContentModel.PROP_EMAIL);
		}

		if (groupName == "Review") {
			recipientsList.add(address);
			logger.debug("Sending mail to " + address);
			templateModel.put("review", "revise");
		} else {
			recipientsList.add(groupName);
			logger.debug("Sending mail to " + groupName);
			// The value here needs to be in the form of list.
	...
	public void prepareReminderMailList(String workflowDescription,
		...
		sendMail(templatePATH, groupName, templateModel, subject);
	 ...
	 * @param packageNodeRef - noderef of the current workflow in progress
	 */
	public void prepareRejectionMail(String workflowDescription,
			String templatePATH, String recipient, String taskId,
			String subject, NodeRef packageNodeRef) {
